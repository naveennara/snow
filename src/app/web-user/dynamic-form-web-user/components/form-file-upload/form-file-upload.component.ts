...
export class FormFileUploadComponent implements OnInit, Field {
  config: FieldConfig;
  group: FormGroup;
  derivedFields: any;
  ...
  constructor(private formDataDistributionService: FormDataDistributionService,
   ...
   ngOnInit() {
    ...
    if (actionType === webUserConstants.formReassignActionType || actionType === webUserConstants.formViewActionType) {
      this.signatureDataValue = webUserConstants.apis.imageView + '/' + this.activatedRoute.snapshot.params.recordId
